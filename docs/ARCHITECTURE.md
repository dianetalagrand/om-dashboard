...
| F | Strategic Pillar | String | Governance & Compliance | **Derived** from `Driver` (fixed mapping in `config/clusters.json`), not entered independently — the two fields overlapped, so only Driver is picked manually. Mapping (resolved 2 Aug 2026): OM Efficiency → Cost Optimisation, OM Compliance → OM Governance & Compliance, OM Optimisation → Expansion & Growth |
| G | Driver | String | OM Compliance | **Renamed from `Cluster` 2 Aug 2026** (see `conversations/2026-08-02-gestione-om-brainstorm.md`) — not a filter, a card tag only (§2.1 in `PRODUCT_SPEC.md`). Enum: OM Compliance, OM Optimisation, OM Efficiency — exactly these 3. Mapped from the old Cluster taxonomy in `config/clusters.json` by semantic/name match: OM Compliance ← old Continuity, OM Efficiency ← old Efficiency, OM Optimisation ← old Evolution (the pairing left over — flag to Diane if not what she meant) |
