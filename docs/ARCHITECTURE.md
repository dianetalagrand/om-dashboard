...
> **Data model superseded (3 August 2026, model decisions closed same day)**: Nathan's domain-reframe proposal is **accepted** — see `docs/proposal/README.md`. The single `om_catalog` table below is replaced by Stream/Assessment/Decision/Forum objects (`docs/proposal/om-maps.html` §5); the `Driver` enum is 3 values now (OM Governance, OM Market Expansion, OM Efficiency), not the 4 here. `DetailSections`'s fixed 5-category model is replaced by a closed 13-function Assessment list (3 fixed: Legal, Fiscal/Finance & Tax, DPO; 10 optional — see `config/clusters.json` → `functions`), each with a state of `asked · in · blocking · n/a`, filed by function with no separate impact tag. `Trigger` is new (absent from the table below): an origin type — a decision taken elsewhere, an outside authority, or OM itself spotting a risk/opportunity — plus a link to the Decision and Forum that set the stream off (`config/clusters.json` → `trigger`). The real content of all 34 streams (status, function assessments, dates) is seeded in `docs/proposal/m1-streams-seed.json`, not the flat table below. Access/Admin/notifications sections below are **not** superseded. Kept for reference until the accepted schema is written up in full.
...
| F | Strategic Pillar | String | Governance & Compliance | **Derived** from `Driver` (fixed mapping in `config/clusters.json`), not entered independently — the two fields overlapped, so only Driver is picked manually. Mapping (resolved 2 Aug 2026): OM Efficiency → Cost Optimisation, OM Compliance → OM Governance & Compliance, OM Evolution → Expansion & Growth. `OM Optimisation` has none yet (placeholder) |
| G | Driver | String | OM Compliance | **Renamed from `Cluster` 2 Aug 2026** (see `conversations/2026-08-02-gestione-om-brainstorm.md`) — not a filter, a card tag only (§2.1 in `PRODUCT_SPEC.md`). Enum (revised same day, now 4 values): OM Compliance, OM Evolution, OM Efficiency, OM Optimisation. Mapped from the old Cluster taxonomy in `config/clusters.json` by direct name match: OM Compliance ← old Continuity, OM Evolution ← old Evolution, OM Efficiency ← old Efficiency. `OM Optimisation` is genuinely new — no prior cluster maps to it, placeholder content in `config/clusters.json` pending Diane's input |
| H | Output Type | String (CSV) | Business Evolution,Market Expansion | Multi-select, comma-separated — a stream can have more than one Output Type. Enum + Driver associations: see `config/clusters.json` (`outputTypes.*.relatedDrivers`) |
